<div class="modal-dialog">
    <div class="modal-content">
        ...
        <form class="form-horizontal" id="ajax-form" method="POST" action="{{ route('weightprice.update',['id'=>$weightprice->id]) }}"
                ...
                <div class="col-md-6">
                    <select name="region_id" id="region_id" class="form-control" required>
                        ...
                        @foreach ($regions as $regions)
                            <option value="{{$regions->id}}" @if($regions->id == $weightprice->region_id) selected @endif>{{$regions->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    <select name="weightoption_id" id="weightoption_id" class="form-control" required>
                        ...
                        @foreach ($weightoptions as $weightoption)
                            <option value="{{$weightoption->id}}" @if($weightoption->id == $weightprice->weightoption_id) selected @endif>{{$weightoption->name}}</option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    <input id="credits" type="text" class="form-control @error('credits') is-invalid @enderror" value="{{$weightprice->credits}}"
