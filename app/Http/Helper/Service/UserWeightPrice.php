...
class UserWeightPriceService
{
    public function WeightOptionListDatatable($parentId = null)
    {
        ...
        $settings = UserWeightPrice::join('regions','regions.id','user_weight_prices.region_id')->join('weightoptions','weightoptions.id','user_weight_prices.weight_option_id')->join('users','users.id','user_weight_prices.user_id')->select('users.*','user_weight_prices.credit','weightoptions.name as wname','regions.name as rname')->get();
